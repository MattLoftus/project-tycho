...
import * as mercuryView from './views/mercury.js';
import * as venusView from './views/venus.js';
...
const MOBILE_SAFE = new Set(['earth', 'mercury', 'venus', 'marsPlanet']);
if (isMobile) {
  document.querySelectorAll('#space-app #nav .nav-group').forEach((group) => {
    const buttons = group.querySelectorAll('button[data-view]');
...
const viewMeta = {
  ...
  mercury:      { sys: 'Sol',         cls: 'G2V',          dist: '---' },
  venus:        { sys: 'Sol',         cls: 'G2V',          dist: '---' },
...
const viewDescriptions = {
  ...
  mercury: 'Mercury \u2014 the smallest planet and closest to the Sun. A heavily cratered, airless world with extreme temperature swings from -180\u00B0C in shadow to 430\u00B0C in sunlight. Its iron core makes up ~85% of its radius, giving it the highest density of any planet. With no atmosphere and no moons, Mercury\u2019s barren surface resembles Earth\u2019s Moon.',
  venus: 'Venus \u2014 Earth\u2019s "twin" in size but radically different in character. A runaway greenhouse effect traps heat beneath dense sulfuric acid clouds, making its surface the hottest in the Solar System at 465\u00B0C. Venus rotates backwards (retrograde) and so slowly that a single day lasts longer than its year. The thick cloud layer rotates 60\u00D7 faster than the surface below.',
